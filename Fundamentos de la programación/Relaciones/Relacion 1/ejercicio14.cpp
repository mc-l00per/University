...
int main() {
	...
	do	{
		cout << "Introduce el numero a descomponer: ";
		cin >> numero;
		if (numero < -999 || numero > 999)	{
			cout << "\nEl numero tiene mas de tres cifras no es un numero correcto. " << endl;
		}
	} while (numero > 999 && numero < -999);

	// CUERPO DEL PROGRAMA Y SALIDA DE DATOS
	cout << "   " << numero / 100 << "   " << (numero / 10) % 10 << "   " << numero % 10 << endl;
