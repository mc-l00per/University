...
const int TOTAL_MAYUSCULA = ('Z' - 'A') + 1;
...
int main() {
	...
	char cadena[TAMANIO], caracter;
	int indice = 0;
	int contador_mayusculas[TOTAL_MAYUSCULA];
	...
	for(char i = 'A'; i <= 'Z'; i++){
		for(int j=0; cadena[j] != '.'; j++)
			if(i == cadena[j])
				contador_mayusculas[indice] += 1;
		indice++;		
	}

	// SALIDA DE DATOS
	indice = 0;
	for(char i = 'A'; i <= 'Z'; i++, indice++){
		if(contador_mayusculas[indice] > 0)
			cout << i << " == " << contador_mayusculas[indice] << endl;				
	}
