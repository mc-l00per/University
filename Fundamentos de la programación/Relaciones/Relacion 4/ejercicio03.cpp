...
class SecuenciaCaracteres{
	...
	public:
		...
		void MostrarCadena(char p_cadena[]){
			for (int i = 0; i < TamCadena(); ++i)
				cout << "   " << p_cadena[i];
			cout << endl;
		}
		bool EsPalindromo(){
		for(int i = 0, j = TamCadena(); i < TamCadena()/2 && j > TamCadena()/2; i++, j--)
		...
		void Invierte(){
			...
			for(int i=0, j=TamCadena()-1; i<=TamCadena() && j>=0; i++, j--){
				cadena_aux[i] = cadena[j];
			}
			if(TamCadena()%2 != 0)
				cadena_aux[(TamCadena()/2)+1] = cadena[(TamCadena()/2)+1];
			cadena_aux[TamCadena()] = '#';
...
void Menu(SecuenciaCaracteres cadena){
	switch(option){
		...
		case 4:
			...
			break;
		case 5:
			cadena.MostrarCadena(cadena.SetCadena());
			break;
	}
	if(option != 0){
		cin.ignore(256,'\n');
