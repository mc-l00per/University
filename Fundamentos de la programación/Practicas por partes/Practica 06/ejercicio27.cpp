...
int main() {
	...
	int entero,tamanio = 1, aux_entero, salida;
	...
	while(aux_entero >= 10){ 					//obtenemos el número con el que tenemos que dividir.
		tamanio *= 10;
		aux_entero /= 10;
	}
	...
	// SALIDA DE DATOS
	for (int i = tamanio; i >= 1; i /= 10) {	//obtenemos los digitos a mostrar
		salida = aux_entero / i;
		aux_entero %= i;
		cout << "   " << salida;
	}	
