...
int main() {
	...
	for (int i = 1; i <= interes; ++i) {
		...
		for (int j = 1; j <= anios; ++j)	{
			capital_total = capital_total + (capital_total * (i / 100.0));
			cout << fixed << setprecision(2) << "\tDinero obtenido en el año número " << j << " = " << capital_total << endl;
		}
	}
	// SALIDA DE DATOS
