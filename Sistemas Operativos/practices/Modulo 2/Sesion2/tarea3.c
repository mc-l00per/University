...
//CAMBIO DE PERMISOS
if(stat("archivo1",&atributos) < 0) { // stat recoje los atributos(conjunto de metadatos. inodo, tipo de fichero ...etc) de archivo1 y 
	// y los copia en la variable &atributos
	printf("\nError al intentar acceder a los atributos de archivo1");
	perror("\nError en lstat");
	exit(EXIT_FAILURE);
}
if(chmod("archivo1", (atributos.st_mode & ~S_IXGRP) | S_ISGID) < 0) { // al archivo uno, le quita un permiso(virgulilla) y le añade otros permisos. 
